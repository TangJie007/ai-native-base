...
| 仓库 | 内容 | 安装 |
|------|------|------|
| **@wetspace/wetspec-cli**（本仓库） | 确定性脚本：`compare` / `change` / `verify` / `doctor` … | `npm install` 或 `pnpm add @wetspace/wetspec-cli` |
...
1. **先**在本项目安装 CLI：`npm install @wetspace/wetspec-cli` 或 `pnpm add @wetspace/wetspec-cli`
2. **再**安装 Skill（Cursor）：
