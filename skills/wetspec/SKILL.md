...
| 实现 | `/wetspec-build` | `src/` `tests/` |
| 验收 | `/wetspec-verify` | `reports/*-verify.json` |
...
**两层测试说明**（须在 AskQuestion 前告知用户）：

| 层级 | 路径 | 框架 |
|------|------|------|
| 单元测试 | `src/<module-slug>/__tests__/` | 用户 DP-0 选定 → `unit_test.command` |
| AC 验收 | `tests/<feature_id>/ac-*.test.js` | 固定 `node:test`，`wetspec verify` 执行 |
...
| `src/<module-slug>/__tests__/` | 模块单元测试（框架见 `unit_test.framework`） |
| `tests/<feature_id>/ac-<ac-id>.test.js` | AC 自动验收（固定 Node `node:test`） |

**模块 slug 映射**：`design.md` 必须写明 PRD 中文模块 → 英文目录，例如 `用户登录` → `user-login`。Spec 仍用 `specs/用户登录/`。

实现规则：
- `test_method: auto/both` 的 AC **必须**有对应测试文件
- 配置与设计一致（如 `SMS_CODE_TTL_SECONDS=600`）
- 每个 AC 测试文件命名：`ac-001.test.js`（AC 编号小写）

### Step B3.5: 单元测试

在 `src/<module-slug>/__tests__/` 按 `unit_test.framework` 编写单元测试，覆盖核心业务逻辑（与 AC 测试分层，不重复堆砌）。
...
wetspec verify <spec_yaml> --root <project_root> \
  --report reports/<feature_id>-verify.json
```

脚本行为：
1. 读取 `acceptance_criteria`（及 legacy `spec.acceptance` 作人工清单）
2. 对 `test_method: auto/both` 的 AC，运行 `tests/<id>/ac-<ac-id>.test.js`
3. 输出逐项 pass/fail/manual 报告
...
wetspec state set specs/ --field verification_report --value reports/<id>-verify.json
wetspec state check specs/ --transition verify-pass
wetspec state set specs/ --field phase --value done
```

同时将 Spec `metadata.status` 更新为 `implemented`。
...
| 选项 label | 动作 |
|------------|------|
| 修复代码后重试（改 src/tests 使 AC 通过） | `verify-fail` → build |
...
- AC 验收（`wetspec verify`）与用户选的单元测试框架无关，AC 层固定 `node --test`
